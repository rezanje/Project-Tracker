...
export function ContentStats({
  ...
  const cells = [
    { n: total, label: 'Total content', tint: 'var(--ink)' },
    { n: scheduled, label: 'Scheduled', tint: 'var(--pop)' },
  ...
  return (
    <div className="card mx-auto mb-5 flex max-w-[1400px] flex-wrap items-stretch p-0">
      ...
      <div className="flex min-w-[180px] flex-[1.4] flex-col justify-center gap-1 border-l border-[var(--line-soft)] px-4 py-3">
...
export function ContentPipeline({ scheduled, posted }: { scheduled: number; posted: number }) {
  ...
  const stages = [
    ...
    { key: 'Script', icon: ScrollText, n: 5, tint: 'var(--ink)', real: false },
    { key: 'Design', icon: Palette, n: 3, tint: 'var(--ink2)', real: false },
    { key: 'Edit', icon: Film, n: 2, tint: 'var(--ink2)', real: false },
    { key: 'Scheduled', icon: CalendarClock, n: scheduled, tint: 'var(--pop)', real: true },
    { key: 'Published', icon: Rocket, n: posted, tint: 'var(--accent)', real: true },
  ]
  return (
    <div className="panel mx-auto mb-5 max-w-[1400px] p-6">
      <h3 className="mb-3 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--ink3)]">
        <FileEdit size={13} /> Content Pipeline
      </h3>
      <div className="flex items-center gap-2 overflow-x-auto">
        {stages.map((s, i) => (
          <div key={s.key} className="flex flex-1 items-center gap-2">
            <div className="min-w-[110px] flex-1 rounded-[14px] border border-[var(--line)] p-2.5">
              <div className="flex items-center gap-1.5">
                <s.icon size={14} style={{ color: s.tint }} />
                <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--ink3)]">{s.key}</span>
              </div>
...
export function ContentRail({
  ...
  return (
    <aside className="hidden w-72 shrink-0 flex-col gap-4 xl:flex">
      ...
      {/* heatmap (static) */}
      <section className="panel p-5">
        <h3 className="mb-2.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--ink3)]">
          Content Heatmap
        </h3>
